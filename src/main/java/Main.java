...
public class Main {
    ...
    public static void main(final String[] args) throws IOException {
        if (args.length == 0) {
            ...
            if (!token.equals("-t") && !token.equals("-lt") && !token.equals("-l") && !token.equals("-t")
                    && !token.equals("-n") && !token.equals("-f") && !token.equals("-p") && !token.equals("-auto")) {
                h(commandLineClient);
                return;
            }
            if (token.equals("-t")) {
    ...
    private static void t(final String[] args, final CommandLineClient commandLineClient) {
        if (args.length != 2) {
            throw new RuntimeException("Uso: -t <termo da procura do torrent>");
        }
    ...
    private static void l(final String[] args, final CommandLineClient commandLineClient) {
        ...
        commandLineClient.l(regexAndDestDir.subtitleSearchTerm, regexAndDestDir.regex,
                regexAndDestDir.destinyDirectory);
    }
    ...
    private static void n(final String[] args, final CommandLineClient commandLineClient) {
        ...
        final RegexForSubPackageAndSubFile regexForSubPackageAndSubFile = RegexUtils
                .getRegexForSubPackageAndSubFile(regex);
    ...
    private static class SubSearchTermRegexAndDestDir {
        ...
        private SubSearchTermRegexAndDestDir(final String subtitleSearchTerm, final String regex,
                final File destinyDirectory) {
    ...
    private static void lt(final String[] args, final CommandLineClient commandLineClient) {
        ...
        commandLineClient.lt(subSearchTermRegexDestDir.subtitleSearchTerm, subSearchTermRegexDestDir.regex,
                subSearchTermRegexDestDir.destinyDirectory);
    }
    ...
    private static SubSearchTermRegexAndDestDir getRegexAnDestDir(final String[] args, final String errorMessage) {
        ...
        final SubSearchTermRegexAndDestDir regexAndDestDir = new SubSearchTermRegexAndDestDir(subtitleSearchTerm, regex,
                destinyDirectory);
        return regexAndDestDir;
    }

    private static void p(final String[] args, final CommandLineClient commandLineClient) {
        if (args.length != 2) {
