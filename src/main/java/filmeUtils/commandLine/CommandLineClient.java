...
public class CommandLineClient implements CommandLine {
    ...
    public CommandLineClient(final LegendasTv legendasTv, final Extractor extract, final OutputListener output) {
    ...
    @Override
    public void lt(final String subtitleSearchTerm, final String regexToApplyOnSubtitlesFiles,
            final File destinantion) {
        final LegendasTv legendasTv = new LegendasTv(output);
        final Downloader downloader = new Downloader(extractor, legendasTv, output);
        output.out("Procurando " + subtitleSearchTerm + " aplicando regex " + regexToApplyOnSubtitlesFiles
                + " salvando em " + destinantion.getAbsolutePath());
