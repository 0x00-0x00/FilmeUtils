...
public class Subtitle {
    ...
    public void search(final String searchTerm, final String subtitleRegex) {
        legendasTv.search(searchTerm, nameAndlink -> {
    ...
    public void downloadNewer(final File destDir, final List<RegexForSubPackageAndSubFile> regexes,
            final List<String> ignoredPackages, final Map<String, List<String>> outSuccessfullPackagesFiles) {
        legendasTv.getNewer(nameAndlink -> {
            final String packageName = nameAndlink.name;
            if (ignoredPackages.contains(packageName)) {
                return;
            }
            final RegexForSubPackageAndSubFile regexMatchingPackageOrNull = RegexUtils
                    .getRegexMatchingPackageOrNull(packageName, regexes);
            if (regexMatchingPackageOrNull == null) {
                return;
            }
            final List<String> outDownloadedSubtitles = downloadSubtitlesMatchingRegexToDir(destDir,
                    regexMatchingPackageOrNull.fileRegex, nameAndlink);
    ...
    public void download(final String searchTerm, final String subtitleRegex, final File destDir) {
        legendasTv.search(searchTerm,
                nameAndlink -> downloadSubtitlesMatchingRegexToDir(destDir, subtitleRegex, nameAndlink));
    }

    private List<String> downloadSubtitlesMatchingRegexToDir(final File destDir, final String subtitleRegex,
            final SubtitlePackageAndLink nameAndlink) {
        ...
        final List<String> filesThatMatches = FileSystemUtils
                .copyFilesMatchingRegexAndDeleteSourceDir(unzippedTempDestination, destDir, subtitleRegex);
    ...
    public void listNewSubtitles() {
        final SubtitleLinkSearchCallback searchListener = nameAndlink -> output.out(nameAndlink.name);
        listNewSubtitles(searchListener);
    }

    public void listNewSubtitles(final SubtitleLinkSearchCallback searchListener) {
    ...
    private File downloadAndExtractToTempDirReturnUnzippedDir(final String link) {
        ...
        try {
            ...
            output.out("Download de pacote de legendas de " + link + " para " + zipTempDestination
            ...
            try {
                ...
                try {
                    ...
                    final FileInputStream fileInputStream = new FileInputStream(zipTempDestination);
                    try {
                        final String fileContents = IOUtils.toString(fileInputStream);
